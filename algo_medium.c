...
/*   Updated: 2026/05/11 03:23:21 by varandri         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static int	member_in_range(t_stack *stack_a, long min, long max)
{
	...
	while (stack_a)
	{
		if (stack_a->index >= (long)min && stack_a->index <= (long)max)
			count ++;
		stack_a = stack_a->next;
	}
	return (count);
